...
$(document).on('click', '#chartInfo tr', function(){
	...
	if($(this).hasClass('selected')){ // 이미 선택된 항목
		...
		$("#ptNm").text($(this).find('td').eq(2).text());
		$("#ptNum").text($(this).find('td').eq(3).text());
		$("#ptBrdt").text($(this).find('td').eq(4).text());
		$("#ptMemo").text($(this).find('td').eq(2).data('memo'));
		$("input[id='" + $(this).find('td').eq(4).data('gender') + "']").prop('checked',true);

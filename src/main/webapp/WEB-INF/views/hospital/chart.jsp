...
<!-- 							</div> -->
					...
					<div id="chartCardBody" class="card-body overflow-auto table-responsive pt-0" style="padding:0px;">
						<input type="hidden" id="chartListPtNum" />
						<input type="hidden" id="chartListChkNum" />
...
// 차트의 환자 번호 (현재 선택 중인 환자의 번호)
const chartListPtNum = document.querySelector('#chartListPtNum');
// 차트의 진료 번호 (현재 선택 중인 진료의 번호)
const chartListChkNum = document.querySelector('#chartListChkNum');
...
//	const ptNum = $('#chartListBody').data('ptnum');
	const ptNum = chartListPtNum.value;
...
$(document).on('DOMSubtreeModified', '#chartListBody', function(e){
	if (!isDataPtNumAdded && chartListPtNum.value /* $('#chartListBody').data('ptnum') */) {

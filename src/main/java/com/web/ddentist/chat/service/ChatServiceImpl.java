...
public class ChatServiceImpl implements IChatService {
	...
	public int systemChat(int chatgrpNum, String chatContent) {
		...
		ChatVO chatVO = new ChatVO();
		chatVO.setEmpId("SYSTEM");
		chatVO.setEmpNm("시스템");
		chatVO.setEmpNum("SYSTEM");
		chatVO.setChatgrpNum(chatgrpNum);
		chatVO.setChatContent(chatContent);
	...
	public List<ChatMemberVO> inviteNewMem(ChatgroupVO chatgrpVO, String[] empNum) {
		...
		for(String num : empNum) {
			ChatMemberVO chatMemVO = new ChatMemberVO();
			chatMemVO.setChatgrpNum(chatgrpNum);
			chatMemVO.setEmpNum(num);
		...
		dataMap.put("chatgrpNum", chatgrpNum);
		dataMap.put("invitedMemList", invitedMemList);
		invitedMemList = chatMapper.listInsertedChatMem(dataMap);
	...
	public List<EmployeeVO> listChatMem(int chatgrpNum) {
		return chatMapper.listChatMem(chatgrpNum);
	}

	@Override
	public ChatMemberVO exitChatgrp(ChatMemberVO chatMemVO) {
		int result = chatMapper.exitChatgrp(chatMemVO);
		if(result == 0) return null;
		ChatMemberVO exitedChatMem = chatMapper.selectExitedMem(chatMemVO);
		exitedChatMem.setChatgrpNum(chatMemVO.getChatgrpNum());
		return exitedChatMem;
	}

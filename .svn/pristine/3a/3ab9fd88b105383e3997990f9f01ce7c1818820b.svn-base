package com.web.ddentist.hospital.manage.empInfo.util;
...
public class ArticlePage<T> {
	...
	public ArticlePage(int total, int currentPage, int size, List<T> content) {
		...
		if (total == 0) {
			...
			//전체 글 수 % 한 화면에 보여질 목록의 행 수   => 0이 아니면 페이지를 1증가
			if (total % size > 0) {
				totalPages++;
			}
			//1) 전체페이지 구하기 끝 ----------------------------------------------
			...
			//2) 시작페이지 구하기 시작 ----------------------------------------------
			//시작페이지번호 = 현재페이지번호 / 페이지(블록)크기*페이지크기 +1
			//1set :    [1][2][3][4][5]다음
			...
			//현재페이지 % 페이지 크기  => 0일때 보정 
			if(currentPage % blockSize == 0) { //보통 0이나오면 페이지블록의 끝 번호이다.
				//페이지크기를 빼줌
				startPage -= blockSize;
			}
			//2) 시작페이지 구하기 끝 ----------------------------------------------
	...
	public boolean hasArticles() {
		return total > 0; //true or false로 리턴이된다.
	}
}

...
//			String filename = "/" + getFolder().replace("\\", "/") + "/" + "s_" + uploadFilename;
			hosVO.setHiImg("/resources/images/stamp/stamp.png");
		}
		...
		if(result < 0) { //등록 실패
			HospitalInfoVO hosInfo = this.hospitalInfoService.getHospitalInfo();
			log.info("hosInfo : {병원정보 실패 : } : " + hosInfo);

			return hosInfo;
		}else {   //등록 성공
			HospitalInfoVO hosInfo = this.hospitalInfoService.getHospitalInfo();
			log.info("hosInfo : {병원정보 성공 } : " + hosInfo);
	...
	public String fileUpload(MultipartFile uploadFile) {
		...
		// 업로드 경로
		String uploadFolder = System.getProperty("user.dir").replace("\\eclipse", "") + "\\workspace\\DDentistProj\\src\\main\\webapp\\resources\\images\\stamp";
		/*
		...
		*/
		File saveFile = new File(uploadFolder, "stamp.png");
		try {
			// 기존 파일 삭제
			if(saveFile.exists()) Files.delete(saveFile.toPath());
	...
	public List<ChairVO> createChair(@RequestBody List<ChairVO> param) {

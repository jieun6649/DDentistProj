	...
	fetch('/hospital/desk/listRegist')
		...
		.then(regList => {
			...
			regList.forEach(function(reg, idx){
				...
				code += '<td data-status="' + reg.regStatus + '">' + reg.regStatus + '</td>';
...
function insertRegist(){
	...
	let regCc = document.querySelector('#regCc').value;
	const byteSize = new Blob([regCc]).size;
	if(byteSize > 300){
		simpleErrorAlert('CC는 300자 이내로 입력해주세요.');
		return false;
	}
...
function getWaitingTimeMinute(){
	...
	for(regTr of registTr){
		...
		const regStatus = regTr.childNodes[2].dataset.status;

		if(regStatus != '접수'){
			regTr.childNodes[5].textContent = '-';
		} else {
			let regTime = regTr.childNodes[6].textContent.split(':');
			let regMinutes = parseInt(regTime[0]) * 60 + parseInt(regTime[1]);

			regTr.childNodes[5].textContent = (nowMinutes - regMinutes) + '분';
		}

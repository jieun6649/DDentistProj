...
//		log.info("result(3) : " + result);
	...
	public String cancelPurchase(PurchaseVO purchaseVO) {
		int result = this.drugMapper.cancelPurchase(purchaseVO);
...
//	<select id="purchasePeriod" parameterType="purchaseVO" resultType="PurchaseVO">
	@Override
	public List<PurchaseVO> purchasePeriod(PurchaseVO purchseVO){
		log.info("purchseVO : " + purchseVO);
		
		List<PurchaseVO> list = this.drugMapper.purchasePeriod(purchseVO);
